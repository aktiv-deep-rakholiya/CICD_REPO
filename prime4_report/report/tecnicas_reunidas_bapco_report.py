...
class TecnicasBapcoSummaryReport(models.TransientModel):
    _name = 'tecnicas.reunidas.bapco.summary.report'
    _inherit = 'common.report'
    _description = 'Tecnicas Reunidas Bapco Report'
    ...
    def generate_report_excel(self):
        ...
        def tecnicas_bapco_value_builder(record):
            ...
            values.update({
                ...
                'kms_new': record.kms or 0,
            })

            return values

        start_values_data = self._group_calendar_records(
            calendar_event, tecnicas_bapco_value_builder
        )
            ...
            for category, records in categorious.items():
                ...
                style_header_left_align_right = workbook.add_format(
                    {'bold': True, 'font_size': 10, 'font_name': 'Arial',
                     'align': 'right', 'valign': 'vcenter', 'right': 1,
                     'left': 1, 'top': 1, 'bottom': 1})
                ...
                style_header2 = workbook.add_format(
                    {'bold': True, 'italic': True, 'font_size': 11, 'font_name': 'Arial',
                     'align': 'center', 'valign': 'vcenter', 'right': 1, 'top': 1, 'left': 1,
                ...
                style_header_center = workbook.add_format(
                    {'bold': True, 'italic': True, 'font_size': 11, 'font_name': 'Arial',
                     'align': 'center', 'valign': 'vcenter', 'text_wrap': 1, 'right': 1,
                     'left': 1, 'top': 1, 'bottom': 1})
                ...
                po_value = []
                for val in records:
                    purchase_value = str(val['purchase'])
                    if purchase_value not in po_value:
                        po_value.append(purchase_value)
                    row += 1
                    row1 += 1
                    ...
                    sheet1.write(row, 12, '', style_center_align_color)
                ...
                sheet1.merge_range(next33_row, 7, next33_row, 11, '', style_header_right_border_align_right)
                ...
                sheet1.merge_range(next39_row, 7, next39_row, 8, '', style_header_no_border_align_right)
                sheet1.merge_range(next39_row, 9, next39_row, 11, '000M-MR-0811-006-FEU',
                                   style_header_right_border_align_right)
                ...
                sheet1.merge_range(row, 3, row, 4, po_ref_number, style_header_left_no_bold)
                ...
                data.append(('%s_%s.xlsx' % (start, category), file_content))
        ...
        return {
            ...
            'view_mode': 'form',
            'res_id': self.id,
            'views': [(self.env.ref('prime4_report.tecnicas_bapco_summary_report_view').id, 'form')],
            'res_model': 'tecnicas.reunidas.bapco.summary.report',
            'target': 'new',
