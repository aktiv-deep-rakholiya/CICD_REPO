...
class AcePlantechSummaryReport(models.TransientModel):
    _name = 'ace.plantech.summary.report'
    _inherit = 'common.report'
    _description = 'ACE Plantech Timesheet Report'
    ...
    def generate_report_excel(self):
        ...
        start_values_data = self._group_calendar_records(
            calendar_event, ace_value_builder
        )
        ...
        for start, categorious in start_values_data_sort:
            for category, records in categorious.items():
                ...
                if self.vendor_id:
                    ...
                    worksheet.merge_range(row, 0, row, 4, 'Name of the Vendor : ' + self.vendor_id.name,
                                       style_header_left1)
                ...
                for val in records:
                    ...
                    if part not in uni:
                        uni.append(part)
                        unique += part + '_'
                    totol_fullday += val['days'] if val['days'] else 0
                    row += 1
                    row1 += 1
                ...
                worksheet.merge_range(next12_row, 0, next12_row, 4, '')
                next2_row = next12_row + 1

                worksheet.set_row(next2_row, 50)

                worksheet.merge_range(next2_row, 0, next2_row, 1, 'Name of Inspector : ' + str(start.split('_')[-1]),
                                   style_header_left)

                if self.inspector_id.employee_signature:
                    worksheet.merge_range(next2_row, 2, next2_row, 4, 'Signature:', style_header_left)

                    # worksheet.insert_bitmap(signature, next2_row, 3, 4 , scale_x=0.9, scale_y=.2 ,y=2.5 )
                else:
                    worksheet.merge_range(next2_row, 2, next2_row, 4, 'Signature:', style_header_left)
                ...
                row = 10
                worksheet.merge_range(row, 0, row, 4, 'Vendor : ' + start.split('_')[0], style_header_left1)
        ...
        return {
            ...
            'view_mode': 'form',
            'res_id': self.id,
            'views': [(self.env.ref('petrofac_report.ace_plantech_summary_report_view').id, 'form')],
            'res_model': 'ace.plantech.summary.report',
            'target': 'new',
