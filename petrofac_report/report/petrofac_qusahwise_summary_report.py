...
class PetrofacQusahwiraSummaryReport(models.TransientModel):
    _name = "petrofac.qusahwira.summary.report"
    _description = "Petrofac Qusahwira Timesheet Report"
    ...
    def generate_report_excel(self):
        ...
        sheet1.insert_bitmap('imagetoadd.bmp', 0, 0,scale_x=.54, scale_y=.22)
        row = 1
        sheet1.write_merge(row, row, 0, 10, '', style_header_new_no_line)
        row = 2
        sheet1.write_merge(row, row, 0, 10, '', style_header_new_no_line)
        row = 3
        sheet1.write_merge(row, row, 0, 10, '', style_header_new_no_line)
        ...
        if self.service_no.name ==True :

         sheet1.write_merge(row,row, 1,10, self.service_no.name, style_header_left)
        else:
         sheet1.write_merge(row, row, 1, 10, '', style_header_left)
        row = 8
        sheet1.write(row, 0,'Name of Inspector', style_header_left)
        sheet1.write_merge(row,row, 1,10, self.inspector_id.name, style_header_left)
        ...
        sheet1.write(row, 4, 'Role', style_header2)
        sheet1.write(row, 5, 'Report Number', style_header2)
        sheet1.write(row, 6, 'Hours', style_header2)
        sheet1.write(row, 7, 'Extra Hrs', style_header2)
        sheet1.write(row, 8, 'Amount', style_header2)
        sheet1.write(row, 9, 'Kms', style_header2)
        sheet1.write(row, 10, 'Rate', style_header2)
        ...
        for vals in start_values_data_sort:
            ...
            for val in vals[1]:
                ...
                sheet1.write(row, 4, calendar_event[0].employee_role.name, style_center_align_wrap)
                sheet1.write(row, 5, val['number'], style_center_align)

                if val['abortive_visit'] == False :

                 sheet1.write_merge(row,row, 6, 6, val['hrs_new'], style_center_align)
                 total_hrs +=val['hrs_new']
                elif val['abortive_visit'] == True:

                 sheet1.write_merge(row,row, 6, 6, 'Abortive', style_center_align)
                 total_abortive +=1
                else:
                 sheet1.write_merge(row, row, 6, 6, '', style_center_align)

                sheet1.write(row, 7, val['extra_hrs_new'], style_center_align)

                sheet1.write(row, 8, '', style_center_align)

                sheet1.write(row, 9, val['kms_new'], style_center_align)
        ...
        sheet1.write(next_row, 5, 'Total', style_left_align)
        
        sheet1.write(next_row, 6, str(total_hrs), style_center_align)
        ...
        sheet1.write(next_row, 10, '', style_center_align)
        ...
        sheet1.write(next1_row, 4, '', style_header)
        sheet1.write(next1_row, 5, 'Abortive Total', style_left_align)

        sheet1.write(next1_row, 6, str(total_abortive), style_center_align)
        ...
        sheet1.write(next1_row, 10, '', style_center_align)
        ...
        sheet1.write(next2_row, 4, '', style_header)
        sheet1.write(next2_row, 5, 'Total Invoice Amount', style_left_align_color)
        ...
        sheet1.write(next2_row, 10, '', style_center_align)

        next3_row = next2_row + 1
        sheet1.write_merge(next3_row,next3_row, 0,1, 'Name of Aprroving Authority:', style_header_left)
        sheet1.write_merge(next3_row,next3_row, 2,4, val['approver'], style_header_left)

        sheet1.write(next3_row, 5, '', style_header)

        sheet1.write_merge(next3_row,next3_row, 6,10, 'Signature of Approving Authority:', style_header_left)
        ...
        sheet1.write(next4_row, 5, '', style_header)
        ...
        sheet1.write(next4_row, 10, '', style_center_align)
        ...
        if calendar_event[0]:
            if calendar_event[0].start:
                ...
                calendar_start_date = ''
            sheet1.write_merge(row,row,0, 10, 'Timesheet for the month of '+calendar_start_date, style_header1)
        else:
        ...
        return {
            ...
            'view_type': 'form',
            'res_id': self.id,
            'res_model': 'petrofac.qusahwira.summary.report',
            'target': 'new',
