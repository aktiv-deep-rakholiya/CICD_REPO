...
class PetrofacKocSummaryReport(models.TransientModel):
    _name = "petrofac.koc.summary.report"
    _description = "Petrofac Koc Timesheet Report"
    ...
    def generate_report_excel(self):
            ...
            sheet1.write_merge(row, row, 1, 10, self.inspector_id.ins_name, style_header_left)
            ...
            sheet1.write(row, 7, 'Kms', style_header2)
            sheet1.write(row, 8, 'Extra Hrs', style_header2)
            ...
            if self.filter_by == 'month':
                if self.month_data:
                    if self.month_data:
                        ...
                        domain = [project, date_compare, date_compare1]#, vendor, inspector

                        if self.env.user.id != 2:
                            logging.info('user_1---->%s', self.env.user.login)
            ...
            for vals in start_values_data_sort:
                ...
                for val in vals[1]:
                    ...
                    sheet1.write(row, 7, val['kms_new'], style_center_align)


                    sheet1.write(row, 8, val['extra_hrs_new'], style_center_align)
            ...
            for level1_user in level1_users:
                partner_ids.append(level1_user.partner_id)
                notification_ids.append((0, 0, {'res_partner_id': level1_user.partner_id.id,
            ...
            if notification_ids:
                attachments = self.env['ir.attachment'].sudo().create({
                    ...
                    'type': 'binary',
                    'datas': out,
                    'res_model': 'petrofac.koc.summary.report',
                    'res_id': self.ids[0],
                    'mimetype': 'application/vnd.ms-excel'
                })
                level1_user.partner_id.message_post(body='Report Has been Generated. Kindly Review it !',
