...
class PetrofacBureauSummaryReport(models.TransientModel):
    _name = 'petrofac.bureau.summary.report'
    _inherit = 'common.report'
    _description = 'Petrofac Bureau Timesheet Report'
    ...
    def generate_report_excel(self):
        ...
        def bureau_value_builder(record):
        ...
        start_values_data = self._group_calendar_records(
            calendar_event, bureau_value_builder
        )
        ...
        for start, categorious in start_values_data_sort:
            for category, records in categorious.items():
                ...
                style_header2 = workbook.add_format(
                    {'font_name': 'Arial', 'bold': True, 'font_size': 10, 'align': 'left', 'valign': 'vcenter', 'text_wrap': True,
                ...
                style_header = workbook.add_format(
                    {'bold': True, 'font_size': 8, 'align': 'center', 'valign': 'vcenter', 'text_wrap': True,
                     'border': 1})
                style_header1 = workbook.add_format({'font_name': 'Arial', 'bold': True, 'font_size': 14, 'align': 'center', 'valign': 'vcenter', 'text_wrap': True, 'border': 1})
                ...
                if self.inspector_id:
                    ...
                    worksheet.merge_range(row, 1, row, 8, self.inspector_id.ins_name, style_header_left)
                ...
                worksheet.write(row, 6, 'Hours', style_header2)
                worksheet.write(row, 7, 'Kms', style_header2)
                worksheet.write(row, 8, 'Extra Hrs', style_header2)
                ...
                for val in records:
                    ...
                    if not val['abortive_visit'] :
                        worksheet.write(row, 6, val['hrs_new'], style_center_align)

                    elif val['abortive_visit'] :
                        worksheet.write(row, 6, 'Abortive', style_center_align)

                    else:
                        worksheet.write(row, 6, '', style_center_align)

                    worksheet.write(row, 7, val['kms_new'], style_center_align)

                    worksheet.write(row, 8, val['extra_hrs_new'], style_center_align)

                end_row = row1

                next3_row = end_row + 2
                worksheet.merge_range(next3_row, 0, next3_row, 1, 'Name of Aprroving Authority:', style_header_left)
                worksheet.merge_range(next3_row, 2, next3_row, 3, val['approver'], style_header_left)

                worksheet.write(next3_row, 4, '', style_header)

                worksheet.merge_range(next3_row, 5, next3_row, 8, 'Signature of Approving Authority:', style_header_left)
                ...
                if calendar_event[0]:
                    if calendar_event[0].start:
                        ...
                        calendar_start_date = ''
                    worksheet.merge_range(row, 0, row, 8, 'Timesheet for the month of ' + calendar_start_date, style_header1)
                else:
        ...
        report_common_method.send_report_notification(
            ...
            self.file_name,
            self.file,
            'petrofac.bureau.summary.report',
            self.id,
        ...
        return {
            ...
            'res_model': 'petrofac.bureau.summary.report',
            'target': 'new',
